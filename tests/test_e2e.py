...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "direct_dep", "direct_dep == 0.0", ["direct_dep == 0.0"], id="simple_pin"
        ),
        pytest.param(
            "direct_dep", "direct_dep==0.0", ["direct_dep==0.0"], id="simple_pin"
        ),
        pytest.param(
            "direct_dep",
            "direct_dep @ http://website.com/package",
            ["direct_dep @ http://website.com/package"],
            id="url_pin",
        ),
        pytest.param(
            "direct_dep",
            'direct_dep @ http://website.com/package ;python_version<"2.7"',
            ['direct_dep @ http://website.com/package ;python_version<"2.7"'],
            id="url_pin",
        ),
        pytest.param(
            "direct_dep", "direct_dep[extra1]", ["direct_dep[extra1]"], id="extras"
        ),
        pytest.param(
            "direct_dep",
            "direct_dep[extra1, extra2]",
            ["direct_dep[extra1, extra2]"],
            id="extras",
        ),
        pytest.param(
            "direct_dep",
            'direct_dep [security,tests] >= 2.8.1, == 2.8.* ; python_version < "2.7"',
            ['direct_dep [security,tests] >= 2.8.1, == 2.8.* ; python_version < "2.7"'],
            id="complicated",
        ),
        pytest.param(
        ...
        pytest.param(
            "direct_dep", "other_dep\ndirect_dep", ["direct_dep"], id="superfluous"
        ),
        pytest.param(
            "django",
            lockfile("pip-compile1.txt"),
            ["django==4.1.7", "sqlparse==0.4.3", "asgiref==3.6.0"],
            id="requirements",
        ),
        pytest.param(
            "asgiref",
        ...
        pytest.param(
            "django",
            lockfile("pip-compile2.txt"),
            ["django==4.1.7", "sqlparse==0.4.3", "asgiref==3.6.0"],
            id="requirements",
        ),
        pytest.param(
            "pydantic",
            lockfile("pip-compile3.txt"),
            [
                "pydantic==2.6.1",
...
def test_dependency_resolution(
    project_dir: ProjectDir, modname: str, lockfile: str, expected_args: list[str]
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "modname", {"modname": "package-a"}, "package-a", ["package_a"], id="simple"
        ),
    ],
)
def test_module_map(
...
def test_package_companions(
    project_dir: ProjectDir,
    modname: str,
    package_companions: dict[str, str],
    lockfile: str,
    expected_args: list[str],
):
...
def test_everything(
    ...
    project_dir.setup(
        lockfile, package_companions=package_companions, module_map=module_map
    )
    assert project_dir.try_to_import(modname) == expected_args
...
# @TODO: Test with custom sys.meta_path finders
...
def test_incorrect_version_installed(
    project_dir: ProjectDir, modname: str, lockfile: str, expected_args: list[str]
):
